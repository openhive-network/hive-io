...
interface MainStore {
  ...
  resetActiveEco: () => void;
  setGlobalAppsData: (data: any) => void;
  setStatsAppsData: (data: any[]) => void;
...
export const useMainStore = create<MainStore>((set, get) => ({
  ...
  initializeApp: async () => {
    ...
    // Fetch global apps data
    try {
      const response = await fetch('https://hivedapps.com/api/global');
      const globalData = await response.json();
      if (globalData?.data) {
        get().setGlobalAppsData(globalData.data);
      }
    } catch (error) {
      console.error('Failed to fetch global apps data:', error);
    }

    // Fetch apps data
    try {
      const response = await fetch('https://hivedapps.com/api/apps');
      const appsData = await response.json();
      if (appsData?.apps) {
        get().setStatsAppsData(appsData.apps);
      }
    } catch (error) {
